  ...
  const indicators = [
    {
      ...
      pattern: /(bit\.ly|tinyurl|goo\.gl|shortened|click here now|urgent.*link|update-account|verify-account|confirm-account|ipv4|click-here)/i,
    },
    {
      category: 'Urgent Language',
      severity: 'medium' as const,
      description: 'Uses urgent or threatening language',
      pattern: /(urgent|immediate action|account.*suspended|verify.*now|act now|limited time|expires|suspended|must.*immediately|unusual activity|unauthorized access)/i,
    },
    {
      category: 'Request for Credentials',
      severity: 'critical' as const,
      description: 'Requests sensitive information like passwords or SSN',
      pattern: /(password|social security|ssn|credit card|bank account|verify.*account|confirm.*identity|update.*payment|reconfirm|re-confirm|authenticate|validate.*account|bank.*details)/i,
    },
    {
      category: 'Spoofed Sender',
      severity: 'high' as const,
      description: 'Sender address may be spoofed or suspicious',
      pattern: /(from:.*@(?!.*\.(com|org|net|edu|gov|support))|reply-to:.*@(?!.*\.(com|org|net|edu|gov))|return-path.*different|display.*name.*mismatch)/i,
    },
    {
      category: 'Generic Greeting',
      severity: 'low' as const,
      description: 'Uses generic greetings instead of your name',
      pattern: /(dear (customer|user|member|sir|madam|valued)|valued (customer|member)|to whom it may concern|account holder|dear user)/i,
    },
    {
      category: 'Poor Grammar',
      severity: 'medium' as const,
      description: 'Contains spelling or grammatical errors',
      pattern: /(\b\w+\s+\1\b|recieve|occured|seperate|definately|priviledge|untill|alot|doesnt|dont|wont|cant|shouldnt)/i,
    },
    {
      category: 'Suspicious Attachments',
      severity: 'high' as const,
      description: 'Contains potentially dangerous file attachments',
      pattern: /attachment.*\.(exe|zip|scr|bat|cmd|vbs|js|jar|ps1|dll|msi|app|dmg)/i,
    },
    {
      category: 'Prize or Money Offer',
      severity: 'high' as const,
      description: 'Offers prizes, money, or too-good-to-be-true deals',
      pattern: /(you.*won|congratulations.*prize|claim.*reward|million dollars|inheritance|lottery|free money|exclusive.*offer|limited.*deal)/i,
    },
    {
    ...
    },
    {
      ...
      severity: 'critical' as const,
      description: 'Uses HTML encoding or obfuscation to hide malicious content',
      pattern: /(&\#|&#x|&#\d+|%[0-9a-f]{2}|\\u[0-9a-f]{4}|entity|encoded|obfuscat)/i,
    },
    {
      category: 'Sense of Urgency with Action',
      severity: 'high' as const,
      description: 'Combines urgency with immediate action requests',
      pattern: /(act now|click immediately|verify|confirm|update|validate|authenticate).{0,50}(urgent|immediately|today|now|limited|expires|expiring)/i,
    },
  ];
